...
const AdminSupervisors = () => {
  ...
  ////FUNCIONES QUE USAN EL SERVICIO FARMSERVICE
  const getListSupervisors = async () => {
    setLoading(true)
    const listado = await supervisorService.getList(findName);
    if (listado)
      setList(listado);
  ...
  const updateSupervisor = async () => {
    ...
    const success = await supervisorService.update(data);
    if (success) {
      reset();
      getListSupervisors();
      setShowEdit(false);
  ...
  const customSubmit = async (dataForm) => {
    ...
    const status = getValues("status");
    const data = { id, identification, name, resolution, status };
    const response = await supervisorService.insert(data);
    if (response.success) {
      reset();
      getListSupervisors();
      toast('Realizado.', { autoClose: 1000 });
  ...
  const getSupervisor = async (recordId) => {
    const record = await supervisorService.get(recordId);
    if (record) {
          ...
          >
            <div className="row">
              <div className="card-title">
                <h4 className="text-secondary" >Datos del supervisor</h4>
              </div>
                        ...
                        {list.map((item, index) => {
                          ...
                          return (
                            <tr key={index} className={item.status ? "" : "table-secondary"}  >
                              ...
                              <td style={{ width: "30px" }}>
                                <a
                                  onClick={() => getSupervisor(item._id)}>
